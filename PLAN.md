...
## Feature 1/EAV-11 — Cleanup/Hardening (Done)
What’s drifting
...
## Feature 2/EAV-12 — Data Type Support (Done)
Goals
...
## Feature 3/EAV-13 — JSON Storage Mode (Done)
  - Scope
...
## Feature 4/EAV-14 — Setup Command (Interactive) (Done)
...
### Feature 4 — Interactive Setup Command (Completed Summary) (Done)
...
## Feature 5/EAV-15 — EavEntities / EavAttributes / EavAttributeSets (Done)

Breaking change notice
- We are renaming the core registry tables to be prefixed with eav_. This affects table names and class names:
  - Tables: eav_attributes (was attributes), eav_attribute_sets (was attribute_sets), eav_attribute_set_attributes (was attribute_set_attributes), and eav_entities (new).
  - Classes: EavAttributesTable (was AttributesTable), EavAttributeSetsTable, EavAttributeSetAttributesTable, EavEntitiesTable.
- Behavior and commands will be updated to reference eav_attributes for the attributes registry (e.g., attribute_id -> eav_attributes.id).
- Bake the CRUD first using Cake conventions, then refactor to add plugin-specific logic (usage badges, guardrails, import/export).

Goals
- Provide prefixed models and tables to avoid collisions and to group all plugin tables in DB tools.
...
- Refactor the baked code to:
  - Add usage badges (e.g., “used in X sets”) on EavAttributes index.
  - Add delete guardrails (prevent deleting an attribute used in any set; require explicit “force” to override).
  - Implement import/export for attribute sets (JSON: set name + members with positions); imports are idempotent (create missing entities/sets/links).

Acceptance
- Models:
  - EavAttributesTable/EavAttributeSetsTable/EavAttributeSetAttributesTable/EavEntitiesTable exist and map to eav_* prefixed tables.
  - Validations and Timestamp behaviors enabled; unique constraints enforced on eav_attributes.name, eav_attribute_sets.name, eav_entities.name.
  - Associations wired as defined above.
- CRUD:
  - Baked CRUD for the four models is present and functional.
  - Join management UI for sets (add/remove attributes, set position) works.
- Compatibility:
  - [EavBehavior](file:///home/paul/dev/cakephp/protech_parts/plugins/Eav/src/Model/Behavior/EavBehavior.php) and commands reference eav_attributes (not attributes) for registry lookups; tests updated accordingly.
- Nice-to-haves (non-blocking but planned):
...
## Feature 6/EAV-16 — Command connection handling (Done)
...
## Feature 7/EAV-17 — Behavior consistency and finder
...
## Feature 8/EAV-6 — Tests (Unit, Integration, and UI) — Target 100% coverage
...
## Feature 10/EAV-2 — Documentation
- Rewrite README with:
...
## Feature 12/EAV-19 — Migrate Native Fields to EAV (New)
...
## Feature 13/EAV-18 — Attribute Sets Import/Export, Usage Badges, and Delete Guardrails
